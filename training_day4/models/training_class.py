...
class TrainingClass(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            vals['name_seq'] = self.env['ir.sequence'].next_by_code('training.class')
    ...
    def set_to_confirmed(self):
        ...
        if not self.name_seq:
            self.name_seq = self.env['ir.sequence'].next_by_code('training.class')
        return res
    
    def action_view_member_ids(self):
        action = self.env['ir.actions.act_window']._for_xml_id('training_day4.training_member_action')
